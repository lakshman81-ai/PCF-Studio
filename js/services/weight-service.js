...
class WeightService {
        ...
        if (pcfType === "VALVE") {
            ...
            if (smartEnabled) {
                ...
                if (component.eps && component.eps.length >= 2) {
                    ...
                    const weight = this._findValveWeight(sizeDN, rating, len, refData);
    ...
    _findValveWeight(dn, rating, length, refData) {
        ...
        for (const row of refData) {
            ...
            const rDN = parseFloat(row[map.size]);
            ...
            if (Math.abs(rDN - dn) < 0.1) {
                ...
                const lenKey = map.length || Object.keys(row).find(k => k.toLowerCase().includes("len")) || "Length";
        ...
        if (bestMatch) {
            return parseFloat(bestMatch[map.weight]); // Return Weight
        }
