...
export class PcfTableController {
    constructor() {
        ...
        this.headers = [
            ...
            "Piping Class", "Rating", "Rigid Type", "Weight (ATTR8)", "Material (ATTR3)", "Wall Thk (ATTR4)",
    ...
    _fetchRowAttrs(rowIdx) {
        ...
        if (pipingClass) {
            ...
            if (pcRow) {
                ...
                const matName = String(pcRow[pcMatKey] || '').trim();
                if (matName) {
                    const matMap = dm.getMaterialMap() || [];
                    const normName = matName.replace(/[\s-]/g, '').toUpperCase();
                    const matEntry = matMap.find(m => {
                        const nd = (m.desc || '').replace(/[\s-]/g, '').toUpperCase();
                        return normName.includes(nd) || nd.includes(normName);
                    });
                    result.material = matEntry?.code || matName;
                }
            }
        }
            ...
            if (pipingClass) {
                ...
                if (pcRow) {
                    ...
                    if (matName) {
                        ...
                        if (ca3Inp && matEntry?.code) ca3Inp.value = matEntry.code;
                        else if (ca3Inp && matName)    ca3Inp.value = matName;
                    }
                }
            }
